...
export const ProductCategory = () => {
  ...
  console.log(responseCategoryApi);
  const [dataMobileProduct, setdataMoblieProduct] = useState([])
  const categoryName = localStorage.getItem('categoryName')
  useEffect(() => {
    instance.get(`/products?category=${categoryName}`)
      .then(res => {
  ...
  const [categoryIdSelected, setCategorySelected] = useState();
  ...
  return (
    <div className='slider-mobilephone-list'>
      ...
      <div>
        <div>
          <h3 className='mobilephone-redirect'><i></i><Link id='home-page' to={'/'}>Trang chủ</Link> &gt;<Link id='mobilephone-link' to={'/mobilephone'}> {categoryName.charAt(0).toUpperCase() + categoryName.slice(1)}</Link></h3>
        </div>
        <div className='product-filter-list-content'>
          <ProductFilterList/>
        </div>
        <div>
          <h2 className='product-list-title' style={{ textTransform: 'uppercase' }}>{categoryName}</h2>
        </div>
        <div className='box-product-content' style={{ display: filteredProducts && filteredProducts.length > 0 ? 'grid' : 'flex' }}>
          {filteredProducts && filteredProducts.length > 0 ? (
            ...
            <div className='title-product-empty'>Hiện không có sản phẩm nào</div>
          )}
        </div>
