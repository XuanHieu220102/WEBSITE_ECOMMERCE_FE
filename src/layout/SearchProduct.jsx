...
export const SearchProduct = () => {
  ...
  useEffect(() => {
    instance.get(`/products?filterName=${searchInput}&page=0&size=20`)
      .then(res => {
  ...
  return (
    <div className='slider-mobilephone-list'>
      ...
      <div>
        <div>
          <h3 className='mobilephone-redirect'><i></i><Link id='home-page' to={'/'}>Trang chủ</Link> &gt;<Link id='mobilephone-link' to={'/mobilephone'}>Tìm kiếm</Link></h3>
        </div>
        <div className='product-filter-list-content'>
          <ProductFilterList/>
        </div>
        <div>
          <h2 className='product-list-title' style={{ textTransform: 'uppercase' }}>Kết quả tìm kiếm cho: {searchInput}</h2>
        </div>
